			...
			if ( isset( $_POST[ '_wpcrm_' . $defaultField['name'] ] ) && trim( $_POST[ '_wpcrm_' . $defaultField['name'] ] ) != '' ) {
				...
				switch ( $defaultField[ 'type' ] ) {
					...
					case 'addcontact': {
						...
						global $post;
						$currentid = $post->ID;
						if( null == wcs_get_page_by_title( $new_title, OBJECT, 'wpcrm-contact' ) ) {
							global $wpdb;
					...
					case 'addorganization': {
						...
						global $post;
						$currentid = $post->ID;
						if( null == wcs_get_page_by_title( $new_org_title, OBJECT, 'wpcrm-organization' ) ) {
							global $wpdb;
							$author_id = get_current_user_id();
							$wpdb->insert($wpdb->posts, array(
								'post_content'  => '',
					...
					case 'addproject': {
						...
						global $post;
						$currentid = $post->ID;
						if( null == wcs_get_page_by_title( $new_title, OBJECT, 'wpcrm-project' ) ) {
							global $wpdb;
							$author_id = get_current_user_id();
							$wpdb->insert($wpdb->posts, array(
								'post_content'  => '',
					...
					case 'addcampaign': {
						...
						global $post;
						$currentid = $post->ID;
						if( null == wcs_get_page_by_title( $new_title, OBJECT, 'wpcrm-campaign' ) ) {
							global $wpdb;
							$author_id = get_current_user_id();
							$wpdb->insert($wpdb->posts, array(
								'post_content'  => '',
...
function saveContactTitle( $post_id ) {
	...
	if ( isset( $_POST['_wpcrm_' . 'contact-first-name'] ) && $_POST['_wpcrm_' . 'contact-first-name'] != '' && isset( $_POST['_wpcrm_' . 'contact-last-name'] ) && $_POST['_wpcrm_' . 'contact-last-name'] != '' ) {
		global $wpdb;
		$first = sanitize_text_field( $_POST['_wpcrm_' . 'contact-first-name'] );
		$last = sanitize_text_field( $_POST['_wpcrm_' . 'contact-last-name'] );
		$title = $first . ' ' . $last;
	...
	if ( isset( $_GET['wpcrm-system-action'] ) ) {
		...
		if ( $action == 'new-project-from-opportunity' ) {
			...
			for ( $field = 1; $field <= $field_count; $field++ ) {
				...
				if ( 'wpcrm-opportunity' === $scope || 'wpcrm-project' === $scope ) {
					$clone_fields[ $scope ][ $name ] = array(
						'name' => $name,
						'key'  => $field,
					);
				}
			...
			/**
			 * If we have wpcrm-opportunity set on the custom fields, loop and compare it
			 * on the wpcrm-project using existing key. If they exists, then we need to pull
			 * the opportunity value and copy it over to project's custom field
			 *
			 * Please note, we are using $_GET['post'] as we are on opportunity's edit page. This key always exists
			 * $post_id is the new project post id which we can use to insert custom field data
			 */
			if ( isset( $clone_fields['wpcrm-opportunity'] ) ) {
				foreach ( $clone_fields['wpcrm-opportunity'] as $key => $value ) {
					if ( isset( $clone_fields['wpcrm-project'][ $key ] ) ) {
						$meta_key   = '_wpcrm_custom_field_id_' . $clone_fields['wpcrm-project'][ $key ]['key'];
						$meta_value = get_post_meta( (int) $_GET['post'], '_wpcrm_custom_field_id_' . $value['key'], true );
