...
function wp_crm_system_import_contacts_process(){
	if( isset( $_FILES[ 'import_contacts' ] ) && isset( $_POST[ 'wpcrm_system_import_contacts_nonce' ] ) ){
		if( wp_verify_nonce( $_POST[ 'wpcrm_system_import_contacts_nonce' ], 'wpcrm-system-import-contacts-nonce' ) ){
			/**
			 * How do we determine on duplicate contact when importing?
			 * 
			 * @since	3.2.7
			 */
			$get_import_setting = get_option( 'wpcrm-settings-duplicate-contact' );
			$import_setting = 'skip'; // default
			if( ! empty( $get_import_setting ) ) {
				$import_setting = $get_import_setting;
			}
			...
			if( empty( $errors ) == true ){
				...
				while( ( $fileop = fgetcsv( $handle, 5000, "," ) ) !== false ) {
					...
					if( $i > 0 ) {
						...
						if( 'skip' === $import_setting && wpcrm_system_import_contacts_duplicate( $email, $fileop ) ) {
						...
						/**
						 * Update contact based on settings
						 */
						} elseif( 'update' === $import_setting ) {
							/**
							 * Buttt....
							 * 
							 * Before we could update, we still need to fetch the ID :/
							 * Check $email if exists, then we use that post ID
							 */
							global $wpdb;
							 ...
							 * that matched the current email to the one we have in DB
							 * 
							 * Then we can update, otherwise, well.. create new record..
							 */
							if ( $results ) {
							...
							//Add user's information to contact fields.
							if ( $contactPrefix != '' ) {
								update_post_meta( $post_id, '_wpcrm_contact-name-prefix', $contactPrefix );
							}
							if ( $contactFirst != '' ) {
								update_post_meta( $post_id,'_wpcrm_contact-first-name', $contactFirst );
							}
							if ( $contactLast != '' ) {
								update_post_meta( $post_id,'_wpcrm_contact-last-name', $contactLast );
							}
							if ( $contactOrg != '' ) {
								update_post_meta( $post_id,'_wpcrm_contact-attach-to-organization', $contactOrg );
							}
							if ( $contactRole != '' ) {
								update_post_meta( $post_id,'_wpcrm_contact-role', $contactRole );
							}
							if ( is_email( $contactEmail ) ) {
								update_post_meta( $post_id,'_wpcrm_contact-email', $contactEmail );
							}
							if ( $contactStreet1 != '' ) {
								update_post_meta( $post_id,'_wpcrm_contact-address1', $contactStreet1 );
							}
							if ( $contactStreet2 != '' ) {
								update_post_meta( $post_id,'_wpcrm_contact-address2', $contactStreet2 );
							}
							if ( $contactCity != '' ) {
								update_post_meta( $post_id,'_wpcrm_contact-city', $contactCity );
							}
							if ( $contactState != '' ) {
								update_post_meta( $post_id,'_wpcrm_contact-state', $contactState );
							}
							if ( $contactZip != '' ) {
								update_post_meta( $post_id,'_wpcrm_contact-postal', $contactZip );
							}
							if ( $contactCountry != '' ) {
								update_post_meta( $post_id,'_wpcrm_contact-country', $contactCountry );
							}
							if ( $contactPhone != '' ) {
								update_post_meta( $post_id,'_wpcrm_contact-phone', $contactPhone );
							}
							if ( $contactMobile != '' ) {
								update_post_meta( $post_id,'_wpcrm_contact-mobile-phone', $contactMobile );
							}
							if ( $contactFax != '' ) {
								update_post_meta( $post_id,'_wpcrm_contact-fax', $contactFax );
							}
							if ( $contactURL != '' ) {
								update_post_meta( $post_id,'_wpcrm_contact-website', $contactURL );
							}
							if ( $contactInfo != '' ) {
								update_post_meta( $post_id,'_wpcrm_contact-additional', $contactInfo );
							}
							if( $contactCategories != '' ) {
								$contactTypes = wp_set_object_terms( $post_id, $contactCategories, 'contact-type' );
								if ( is_wp_error( $contactTypes ) ) {
							...
							if( isset( $custom_fields ) && is_array( $custom_fields ) ){
								foreach( $custom_fields as $id => $value ){
									update_post_meta( $post_id, '_wpcrm_custom_field_id_' . $id, $value );
								}
							}
...
function wpcrm_system_import_contacts_duplicate( $email, $fields ){
	...
	return apply_filters( 'wp_crm_system_import_contacts_duplicate', $create_contact, $email, $fields );
}