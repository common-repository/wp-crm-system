					...
					<div class="wp-crm-email-group">
						...
						if ( $filterOrg != '' ) {
							...
							while ( $loop->have_posts() ) :
								...
								if ( $posts ) {
									?>
									<select class="wp-crm-email wp-crm-email-recipient wp-crm-email-has-filter" name="wpcrm-email-recipients[]" id="wpcrm-email-recipients" multiple>
									<?php
							...
							$term_id = array();
							$class = 'wp-crm-email-no-filter';
							if ( $filter_cats == 'yes' ) {
								$class = 'wp-crm-email-has-filter';
								foreach ( $terms as $term ) {
									if ( get_option( $term->slug . '-email-filter' ) == 'yes' ) {
										$term_id[] = $term->slug;
									}
...
function wpcrm_send_email() {
	if ( isset( $_POST['wpcrm_email_send'] ) ) {
		...
		if ( ! isset( $_POST['wpcrm-email-recipients'] ) || ! isset( $_POST['wpcrm-email-subject'] ) || ! isset( $_POST['wpcrm-email-message'] ) || ! isset( $_POST['wpcrm-email-from-name'] ) || ! isset( $_POST['wpcrm-email-from-address'] ) ) {
			...
			$message    = wp_kses_post( wpautop( $_POST['wpcrm-email-message'] ) );
