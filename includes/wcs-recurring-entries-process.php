	...
	if( $wp_crm_system_post ) // if data is being sent
	{
		...
		if( isset( $_POST['add_new_entry'] ) && $_POST['add_new_entry'] == 1 && wp_verify_nonce( $_POST['wp_crm_system_recurring_entry_nonce'], 'wp-crm-system-recurring-entry-nonce' ) ) {
			if( !isset( $_POST['project_task_id'] ) ){
				...
				$page_id = sanitize_text_field( $_POST['project_task_id'] );
			}
			$start_date	= wp_crm_system_process_datetime( sanitize_text_field( $_POST['start_date'] ) );
			$end_date	= wp_crm_system_process_datetime( sanitize_text_field( $_POST['end_date'] ) );
			$new_entry = $wpdb->insert( $wpcrm_system_recurring_db_name,
				array(
					...
					'number_per_frequency'	=> sanitize_text_field( $_POST['number'] )
					)
				);
				if( $new_entry ) {
					$url = 'admin.php?page=wpcrm-settings&tab=recurring&subtab=recurring-entries&entry-added=success';
				} else {
					$url = 'admin.php?page=wpcrm-settings&tab=recurring&subtab=recurring-entries&entry-added=failed';
				}
			...
			if( isset( $_POST['edit_entry'] ) && wp_verify_nonce( $_POST['wp_crm_system_recurring_entry_nonce'], 'wp-crm-system-recurring-entry-nonce' ) ) {
				...
				$edit_entry = $wpdb->update( $wpcrm_system_recurring_db_name,
					...
					array(
						'id' => sanitize_text_field( $_POST['edit_entry'] )
					)
				);
			if( false !== $edit_entry ) {

				$url = 'admin.php?page=wpcrm-settings&tab=recurring&subtab=recurring-entries&entry-edited=success';
			} else {
				$url = 'admin.php?page=wpcrm-settings&tab=recurring&subtab=recurring-entries&entry-edited=failed';
			}
			wp_redirect( $url ); exit;
		}
		...
		if( isset( $_POST['delete_entry'] ) && wp_verify_nonce( $_POST['wp_crm_system_recurring_entry_nonce'], 'wp-crm-system-recurring-entry-nonce' ) ) {
			...
			if( $delete_entry ) {
				$url = 'admin.php?page=wpcrm-settings&tab=recurring&subtab=recurring-entries&entry-deleted=success';
			} else {
				$url = 'admin.php?page=wpcrm-settings&tab=recurring&subtab=recurring-entries&entry-deleted=failed';
			}
			wp_redirect( $url ); exit;
		}
	}
