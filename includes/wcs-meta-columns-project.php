...
/**
 * Add new filter dropdown on Projects
 *
 * @since   1.2
 */
add_action( 'restrict_manage_posts', 'wpcrm_system_project_filter_by_organization' );
function wpcrm_system_project_filter_by_organization() {
	global $pagenow;
	$type = isset( $_GET['post_type'] ) ? sanitize_text_field( $_GET['post_type'] ) : '';
	if ( 'wpcrm-project' === $type && is_admin() && 'edit.php' === $pagenow ) {
		...
		/** Current values */
		$current_v = isset( $_GET['progress'] ) ? trim( sanitize_text_field( $_GET['progress'] ) ) : '';
...
add_filter( 'parse_query', 'wpcrm_system_projects_orgs_filter' );
function wpcrm_system_projects_orgs_filter( $query ) {
	global $pagenow;
	$type = isset( $_GET['post_type'] ) ? sanitize_text_field( $_GET['post_type'] ) : '';
	if ( 'wpcrm-project' === $type && is_admin() && $pagenow === 'edit.php' ) {

		/** Set query for progress */
		if ( isset( $_GET['progress'] ) ) {
			if ( '' !== $_GET['progress'] ) {
				$query->query_vars['meta_key']   = '_wpcrm_project-progress';
				$query->query_vars['meta_value'] = sanitize_text_field( $_GET['progress'] );
			}
		}
		...
		if ( isset( $_GET['status'] ) ) {
			if ( '' !== $_GET['status'] ) {
				$query->query_vars['meta_key']   = '_wpcrm_project-status';
				$query->query_vars['meta_value'] = sanitize_text_field( $_GET['status'] );
			}
		}
	}
}
