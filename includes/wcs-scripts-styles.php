...
function wpcrm_scripts_styles( $hook ) {
	...
	wp_localize_script(
		'wp-crm-system-datepicker',
		'wpcrm_system_datepicker_vars',
		array(
	...
	if ( $active_page == 'wpcrm-email' ) {
		...
		wp_enqueue_style( 'select2', WP_CRM_SYSTEM_PLUGIN_URL . '/assets/dist/css/select2.min.css', array(), WP_CRM_SYSTEM_VERSION, false );
		...
		wp_enqueue_script( 'wpcrm-system-general', WP_CRM_SYSTEM_PLUGIN_URL . '/assets/dist/scripts/wp-crm-system-general.js', array( 'jquery' ) );
		wp_localize_script(
			'wpcrm-system-general',
			'wp_crm_system_general',
			array(
				'ajax_url'    => admin_url( 'admin-ajax.php' ),
				'wpcrm_nonce' => wp_create_nonce( 'wpcrm-nonce' ),
				'i18n' => array(
					'recipient' => __( 'Please select recipient', 'wp-crm-system' ),
				)
			)
		);
	}
	...
	if ( in_array( $post_type, $wp_crm_posts, true ) || 'wpcrm-settings' === $active_page ) {
		wp_enqueue_script( 'wp_crm_system_quick_add_modal', WP_CRM_SYSTEM_PLUGIN_URL . '/assets/src/scripts/wp-crm-system-quick-add-modal.js', array( 'jquery' ), WP_CRM_SYSTEM_VERSION, false );
		wp_localize_script(
			'wp_crm_system_quick_add_modal',
			'wp_crm_system_quick_add_modal',
			array(
				'plus_icon' => array(
					'path'     => WP_CRM_SYSTEM_URL . '/assets/src/images/plus-33-256.png',
					'desc' => array(
						'organization' => __( 'Add new organization', 'wp-crm-system' ),
						'contact'      => __( 'Add new contact', 'wp-crm-system' ),
						'project'      => __( 'Add new project', 'wp-crm-system' ),
						'campaign'     => __( 'Add new campaign', 'wp-crm-system' ),
					),
				),
	...
	if ( 'wpcrm-contact' === $post_type ) {
		wp_enqueue_script( 'wp_crm_system_contact', WP_CRM_SYSTEM_PLUGIN_URL . '/assets/src/scripts/wp-crm-system-contact.js', array( 'jquery' ), WP_CRM_SYSTEM_VERSION, false );
		wp_localize_script(
			...
			array(
				'i18n'      => array(
					'error_message' => array(
						...
						'url'         => __( 'Please enter a valid URL adress. e.g. https://www.google.com', 'wp-crm-system' ),
					),
				),
				'plus_icon' => array(
					'path'     => WP_CRM_SYSTEM_URL . '/assets/src/images/plus-33-256.png',
					'org_desc' => __( 'Add new organization', 'wp-crm-system' ),
				),
				'nonce'     => wp_create_nonce( 'ajax-nonce' ),
				'ajax_url'  => admin_url( 'admin-ajax.php' ),
			)
