...
function wp_crm_system_ajax_address_book() {
	...
	if ( '' != trim( $thumbnail ) )
		echo '<a href="' . get_edit_post_link( $contact_id ) . '" class="wpcrm-system-help-tip wpcrm-gravatar-wrapper" title="' . __( 'Click to view contact', 'wp-crm-system' ) . '">' . $thumbnail . '</a>';
...
function wpcrm_system_dashboard_contacts_js($hook){
	wp_enqueue_script( 'wp_crm_system_dashboard_address_book', WP_CRM_SYSTEM_PLUGIN_URL . '/assets/dist/scripts/dashboard-address-book.js', array( 'jquery' ), WP_CRM_SYSTEM_VERSION, false );
