...
function wprcm_system_contact_columns_content( $column, $post_id ) {
	...
	switch( $column ) {
		...
		case 'photo' :
			...
			$thumbnail = get_the_post_thumbnail( $post_id, array( 96, 96 ), array( 'class' => 'avatar photo' ) );
			...
			if ( empty( $email ) && empty( $thumbnail ) ){
				 ...
				 * If no email found
				 * 
				 * Use dummy one to generate default avatar image
				 */
				echo get_avatar( 'dummy@wpcrm.dummy' );
