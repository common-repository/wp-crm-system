...
function wp_crm_system_show_project_form() {
	...
	<form method="post">
		...
		<div class="wp-crm-one-fourth"><?php require plugin_dir_path( __FILE__ ) . '/options/category-type.php'; ?></div>
...
function wp_crm_system_process_project_form() {
	...
	&& wp_verify_nonce( sanitize_key( $_POST['project_report_nonce'] ), 'check_project_report_nonce' ) ) {
		...
		/**
		 * If we have category sorting
		 * 
		 * Merge them on the existing arguments
		 */
		if( isset( $_POST['wp-crm-system-tax-name'] ) && isset( $_POST['wp-crm-system-type'] ) ) {
			if( 'all' === $_POST['wp-crm-system-type'] ) {
				/**
				 * If the chosen category type is 'all'
				 * 
				 * Do nothing! This will skipped the category filter
				 */
			} elseif( '' === $_POST['wp-crm-system-type'] ) {
				/**
				 * If the chosen category is empty, this probably "NOT SET"
				 * 
				 * We need to filter all data that don't have any category
				 */
				$args = array_merge( $args, array(
					'tax_query' => array(
						array(
							'taxonomy' => sanitize_text_field( $_POST['wp-crm-system-tax-name'] ),
							'field'    => 'slug',
							'terms'    => array(),
							'operator' => 'NOT EXISTS'
						)
				...
				$args = array_merge( $args, array(
					'tax_query' => array(
						array(
							'taxonomy' => sanitize_text_field( $_POST['wp-crm-system-tax-name'] ),
							'field'    => 'slug',
							'terms'    => array( sanitize_text_field( $_POST['wp-crm-system-type'] ) ),
						)
					)
				) );
			}
		...
		if ( $wpcposts ) {
			...
			$project_report .= '<th><strong>' . esc_attr_x( 'Assigned', 'wp-crm-system' ) . '</strong></th>';
			$project_report .= '<th><strong>' . esc_attr_x( 'Project Type', 'wp-crm-system' ) . '</strong></th></tr>';
			foreach ( $wpcposts as $wpcpost ) {
				...
				if ( '' !== $close_output && is_int( $close_output ) ) {
				...
				$terms = isset( $_POST['wp-crm-system-tax-name'] ) ? get_the_terms( $wpcpost->ID, sanitize_text_field( $_POST['wp-crm-system-tax-name'] ) ) : '';
				if( ! empty( $terms ) ) {
					$categories = array();
					foreach( $terms as $term ) {
						$categories[] = $term->name;
					}
					$project_report .= '</td><td>' . implode( ', ', $categories );
				}
