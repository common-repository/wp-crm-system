...
function wp_crm_system_show_task_form() {
	...
	<form method="post">
		...
		<div class="wp-crm-first wp-crm-one-fourth"><?php require plugin_dir_path( __FILE__ ) . '/options/category-type.php'; ?></div>
...
function wp_crm_system_process_task_form() {
	...
	&& wp_verify_nonce( sanitize_key( $_POST['task_report_nonce'] ), 'check_task_report_nonce' ) ) {
		...
		if ( $wpcposts ) {
			...
			$task_report .= '<th><strong>' . esc_attr_x( 'Assigned', 'wp-crm-system' ) . '</strong></th>';
			$task_report .= '<th><strong>' . esc_attr_x( 'Opportunity Type', 'wp-crm-system' ) . '</strong></th></tr>';
			foreach ( $wpcposts as $wpcpost ) {
				...
				$terms = get_the_terms( $wpcpost->ID, sanitize_text_field( $_POST['wp-crm-system-tax-name'] ) );
				if( ! empty( $terms ) ) {
					$categories = array();
					foreach( $terms as $term ) {
						$categories[] = $term->name;
					}
					$task_report .= '</td><td>' . implode( ', ', $categories );
				}
