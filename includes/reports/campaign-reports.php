...
function wp_crm_system_process_campaign_form() {
	...
	&& wp_verify_nonce( sanitize_key( $_POST['campaign_report_nonce'] ), 'check_campaign_report_nonce' ) ) {
		...
		if ( $wpcposts ) {
			...
			$campaign_report .= '<th><strong>' . esc_attr_x( 'Assigned', 'wp-crm-system' ) . '</strong></th>';
			$campaign_report .= '<th><strong>' . esc_attr_x( 'Campaign Type', 'wp-crm-system' ) . '</strong></th></tr>';
			foreach ( $wpcposts as $wpcpost ) {
				...
				if( ! empty( $terms ) ) {
					...
					$campaign_report .= '</td><td>' . implode( ', ', $categories );
				}
