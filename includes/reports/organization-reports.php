...
function wp_crm_system_process_organization_form() {
	...
	&& wp_verify_nonce( sanitize_key( $_POST['organization_report_nonce'] ), 'check_organization_report_nonce' ) ) {
		...
		if ( $wpcposts ) {
			...
			$organization_report .= '<th><strong>' . esc_attr_x( 'Organization Type', 'wp-crm-system' ) . '</strong></th>';
			foreach ( $wpcposts as $wpcpost ) {
				...
				if( isset( $_POST['wp-crm-system-tax-name'] ) ) {
					...
					if( ! empty( $terms ) ) {
						...
						$organization_report .= '</td><td>' . implode( ', ', $categories );
					}
				}
