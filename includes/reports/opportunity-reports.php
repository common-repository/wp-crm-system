...
function wp_crm_system_process_opportunity_form() {
	...
	&& wp_verify_nonce( sanitize_key( $_POST['opportunity_report_nonce'] ), 'check_opportunity_report_nonce' ) ) {
		...
		if ( $wpcposts ) {
			...
			$opportunity_report .= '<th><strong>' . esc_attr_x( 'Assigned', 'wp-crm-system' ) . '</strong></th>';
			$opportunity_report .= '<th><strong>' . esc_attr_x( 'Opportunity Type', 'wp-crm-system' ) . '</strong></th></tr>';
			foreach ( $wpcposts as $wpcpost ) {
				...
				if( ! empty( $terms ) ) {
					...
					$opportunity_report .= '</td><td>' . implode( ', ', $categories );
				}
